...
The JSON MUST use this exact schema. Every field is required:
{
  ...
  "memory": "(string) Your own short-term memory of the conversation, max 60 characters. A quick note-to-self on the CURRENT topic or open agenda right now — an offer sitting on the table, a question you're waiting to be answered, something you're suspicious of. Rewrite it fresh every turn from scratch; never just repeat the previous one unless the topic genuinely hasn't moved on.",
  "player_options": [
    {
...
- Each pre_response uses the same schema as the top-level (npc_dialogue, face, arms, bubble, body_anim, mood, patience, trade_state, revealed_item, trade_result, memory, player_options) but does NOT include its own pre_responses. Its own player_options must follow the same fixed friendly/shrewd/aggressive/deceptive order too.
...
- memory is YOUR working notes, not narration to the player — write it like a private aside (e.g. "He's offered the watch for my six-shooter", "Still waiting to hear his real price", "He keeps dodging what he actually wants"). Update it every single turn, including every pre_response, to whatever's most current — an open trade offer on the table, an unanswered inquiry, a demand you're mulling over, or general small talk if nothing specific is pending. This is exactly what keeps you from losing track of what's actually being discussed, so be concrete about the real subject (name the item, the number, the ask) rather than a vague mood description.
